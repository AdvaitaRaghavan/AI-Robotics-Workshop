...
int setMotorRunning(uint8_t motorState) {
 ...
 int setRPM(int motor, float RPM) {
    ...
    attinySlaveArrayBoard[1] = RPM*50 ;  // Param1 - rpm(L) 
    ...
    if (Wire.endTransmission () == 0) { // Receive 0 = success (ACK response) 
       // Serial.println("i2c Write to 0x12 Sucessfull");
       Serial.print("Motor = "); Serial.print(String(motor));
       Serial.print(" Speed = ");Serial.println(String(RPM*50));
        return 0;
    }
    else {
        Serial.println("i2c Write Failed");
...
void readSensorValues()
{
    ...
    for (byte i = 0; i < 9; i++) {     
        if (sensor[0] == pidErrorMap[i][0] && 
            sensor[1] == pidErrorMap[i][1] && 
            sensor[2] == pidErrorMap[i][2] && 
            sensor[3] == pidErrorMap[i][3] && 
...
void loop() {
  setMotorRunning(HIGH);
    readSensorValues();
    Serial.print("Error = "); Serial.println(String(error));
    calculatePID();
    motorControl();
}
